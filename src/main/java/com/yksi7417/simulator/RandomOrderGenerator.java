...
public class RandomOrderGenerator implements IRandomOrderGenerator {
	...
	public void run() {
		TimerTask task = new PeriodicTimerEvent(
				watch, 15 * MINUTES, () -> sendRandomOrder());
	...
	void sendRandomOrder(){
		...
		this.limitOrderBook.placeOrder(new LimitOrder(side, size, price, watch.now()));
	}
